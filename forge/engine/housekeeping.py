...
image name from the queued row (see forge.domain.retention).
"""
...
logger = logging.getLogger("forge.housekeeping")
...
async def sweep_images(settings: Settings) -> list[str]:
    ...
    removed = []
    present = await containers.list_forge_images(settings.network)
