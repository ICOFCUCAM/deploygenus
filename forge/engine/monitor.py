...
from forge.engine import health
    ...
    async def _check_site(self, project) -> int:
        ...
        return await self._record(
            ...
            down_detail=(
                ...
                f"{problem}. Check `forge logs {deployment.short_id}` and the "
                "container's own output; roll back with "
                f"`forge promote {project.slug} '#<number>'`."
            ),
        ...
        if percent >= limit:
            await self.alerts.fire(
                ...
                detail=(
                    ...
                    "when it fills. `forge doctor` shows what is using it; old "
