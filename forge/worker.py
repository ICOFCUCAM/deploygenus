...
from forge.engine import backup, housekeeping, pipeline, processes, scheduler, service
    ...
    async def _alert_on_deploy(self, deployment) -> None:
        ...
        await self._alerts.event(
            ...
            detail=(
                ...
                f"Production was not touched. `forge logs {deployment.short_id}`"
            ),
            level="critical",
    ...
    async def _alert_on_job(self, process, run) -> None:
        ...
        if run.status.value in {"failed", "timed_out"}:
            ...
            await self._alerts.fire(
                ...
                detail=(
                    f"{run.detail or ''}\n"
                    f"`forge runs {project.slug} {process.name} --output`"
                ).strip(),
                project=project.slug,
            )
        ...
        if killed:
            ...
            await self._alerts.event(
                ...
                detail=(
                    ...
                    "the project's stop timeout: `forge project set <project> "
