...
#: neither a project slug nor a volume name, so `forge_<slug>_<name>` is
...
def docker_volume_name(project_slug: str, volume: str) -> str:
    """`balancevid` + `recordings` -> `forge_balancevid_recordings`.
    ...
    """
    return f"forge{_SEPARATOR}{project_slug}{_SEPARATOR}{volume}"
...
def draining_name(name: str, *, deadline: int) -> str:
    """`forge-blog-mailer-0` -> `forge-blog-mailer-0.draining.1790000000`."""
