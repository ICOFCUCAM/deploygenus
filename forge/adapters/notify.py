...
the same setting works for either without Forge knowing which it is talking
...
async def send(url: str, *, title: str, detail: str, level: str, **fields: str) -> bool:
    ...
    body = {
        "text": message,
        "content": message,
        "forge": {"title": title, "detail": detail, "level": level, **fields},
    }
    try:
