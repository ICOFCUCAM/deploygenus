...
from deploypro.engine.backup import latest, prune, taken_today
...
def test_keeps_the_newest_and_removes_the_rest(tmp_path):
    dest = make(
        tmp_path,
        "deploypro-20260920T030000Z",
        "deploypro-20260921T030000Z",
        "deploypro-20260922T030000Z",
    )
    removed = prune(dest, keep=2)
    assert [p.name for p in removed] == ["deploypro-20260920T030000Z"]
...
def test_an_unfinished_backup_never_counts_as_the_latest(tmp_path):
    dest = make(
        tmp_path, "deploypro-20260921T030000Z", "deploypro-20260922T030000Z.partial"
    )
    assert latest(dest).name == "deploypro-20260921T030000Z"
...
def test_other_files_in_the_directory_are_left_alone(tmp_path):
    dest = make(tmp_path, "deploypro-20260921T030000Z", "my-notes")
...
def test_knows_whether_today_is_done(tmp_path):
    dest = make(tmp_path, "deploypro-20260922T030000Z")
