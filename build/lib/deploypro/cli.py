...
def migrations_dir() -> Path:
    """Where the schema migrations are.

    Not simply "next to the code": in a development checkout the package runs
    from the repository and db/migrations is beside it, but in the container
    the package is installed into site-packages, where there is no db/ at all.
    Resolving relative to __file__ there found no migrations, so `migrate`
    reported "up to date" against an empty database — the first real install
    started with no tables. The image now says where they are, and finding
    none is an error rather than a success.
    """
    candidates = []
    if os.environ.get("DEPLOYPRO_MIGRATIONS_DIR"):
        candidates.append(Path(os.environ["DEPLOYPRO_MIGRATIONS_DIR"]))
    candidates += [
    ...
    raise DeployProError(
        f"No migration files found (looked in {looked}). Refusing to report the "
        "database as up to date: set DEPLOYPRO_MIGRATIONS_DIR to the directory "
        "holding 0001_core.sql."
    )
...
async def cmd_migrate(args: argparse.Namespace, settings: Settings) -> None:
    ...
    pending = [p for p in sorted(source.glob("*.sql")) if p.name not in applied]
    if not pending:
