...
class DeployProError(Exception):
...
class NotFound(DeployProError):
    status_code = 404
    code = "not_found"


class Conflict(DeployProError):
...
class InvalidRequest(DeployProError):
    status_code = 400
    code = "invalid_request"


class Unauthorized(DeployProError):
    status_code = 401
    code = "unauthorized"


class DetectionFailed(DeployProError):
...
class BuildFailed(DeployProError):
...
class DeployFailed(DeployProError):
