...
2. A `deploypro.json`. The repository has described itself in the platform's own
   terms.
3. Project settings stored in DeployPro. The owner overrode detection in the UI.
...
def detect(app_dir: Path, overrides: Overrides | None = None) -> BuildPlan:
    ...
    manifest = _read_json(app_dir / "deploypro.json")
    if manifest:
    ...
    raise DetectionFailed(
        "Could not work out how to build this directory. DeployPro looked for a "
        "Dockerfile, deploypro.json, package.json, requirements.txt, pyproject.toml, "
        ...
        "otherwise add a Dockerfile and DeployPro will use it unchanged."
    )
...
def _merge_manifest(over: Overrides, manifest: dict) -> Overrides:
    """`deploypro.json` fills in whatever the project settings left unset.

    Project settings win: a value typed into DeployPro is a deliberate override of
...
def _detect_node(app_dir: Path, pkg: dict, over: Overrides, port: int) -> BuildPlan:
    ...
    raise DetectionFailed(
        "This looks like a Node project, but there is no framework DeployPro "
...
def _detect_next(
    ...
    return gen.node_server(
        ...
        + (
            ...
            "DeployPro switches automatically once it is set.",
        ),
    )
...
def _detect_python(app_dir: Path, over: Overrides, port: int) -> BuildPlan:
    ...
    raise DetectionFailed(
        "This looks like a Python project, but DeployPro could not find the "
