...
Almost nothing on a DeployPro host needs a backup. Images rebuild from git,
...
encrypted with DEPLOYPRO_MASTER_KEY; a backup that carried the key would carry
...
ADVISORY_LOCK = 0x466F726765  # "DeployPro"
...
async def _take(settings: Settings, dest: Path) -> BackupResult:
    ...
    try:
        how = await pgdump.dump(
            settings.database_url, work / "deploypro.dump", network=settings.network
        )
        ...
        files = {
            "deploypro.dump": await asyncio.to_thread(_describe, work / "deploypro.dump")
        }
        ...
        manifest = {
            "format": 1,
            "taken_at": stamp,
            "database": {"file": "deploypro.dump", "taken_with": how},
            "volumes": exported,
            "files": files,
            "master_key_included": False,
            "restore": "See README.md, Backups, in the DeployPro repository.",
        }
