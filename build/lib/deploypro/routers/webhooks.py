...
from deploypro.domain.errors import InvalidRequest, NotFound, Unauthorized
from deploypro.domain.models import DeploymentTrigger, Project
from deploypro.engine import service
from deploypro.repositories import projects as project_repo
from deploypro.routers.deployments import _out
from deploypro.routers.schemas import WebhookAccepted

logger = logging.getLogger("deploypro.webhooks")
...
async def reveal_secret(project_slug: str) -> dict[str, str]:
    ...
    raise NotFound(
        ...
        f"`deploypro webhook {project_slug}` on the host to print it."
    )
